...
interface TeamAnswerFormProps {
  selectedTeam: Team
}

export default function TeamAnswerForm({ selectedTeam }: TeamAnswerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateQuestionId(formData.questionId)) {
      setMessage({ text: 'Fragen-ID muss zwischen 1 und 3 liegen', type: 'error' })
      return
    }
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-4 sm:p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
        <p className="text-sm text-green-800 dark:text-green-200 text-center">
          💡 Gib deine Antwort ein und sammle Punkte für {selectedTeam.name}!
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Wähle eine der 3 Fragen für dein Team
          </p>
        </div>
